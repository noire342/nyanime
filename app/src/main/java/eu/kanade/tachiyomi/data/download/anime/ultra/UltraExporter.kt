...
internal object UltraExporter {
    ...
    suspend fun export(context: Context, input: Uri, output: File, progress: suspend (Int) -> Unit): Media {
        ...
        withContext(Dispatchers.Main.immediate) {
                ...
                .addListener(object : Transformer.Listener {
                    override fun onCompleted(composition: Composition, exportResult: ExportResult) {
                        if (exportResult.videoFrameCount > 0) {
                            ...
                            complete.completeExceptionally(IllegalStateException("Ultra non ha prodotto fotogrammi"))
                        }
                    }
            ...
            val item = EditedMediaItem.Builder(MediaItem.fromUri(input))
                .setRemoveAudio(true)
                .setEffects(Effects(emptyList(), listOf(UltraGlEffect())))
                .build()
            try {
                ...
                while (!complete.isCompleted) {
                    ...
                    progress(if (state == Transformer.PROGRESS_STATE_AVAILABLE) holder.progress else 0)
                    delay(500)
                }
                complete.await()
            } finally {
        ...
        require(
            kotlin.math.abs(result.durationMs - original.durationMs) <=
                (original.durationMs / 100).coerceIn(100L, 1500L),
